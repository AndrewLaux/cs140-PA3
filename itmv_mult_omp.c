 ...
 * BLOCK_GUIDED. These constants are defined in itmv_mult_omp.h They correspond
 * to OpenMP's omp_sched_static with block size equal to number of iterations
 * divided by number of threads, omp_sched_static with basic chunk size equal to
 * chunksize omp_sched_dynamic with basic chunk size equal to chunksize. Type
 * omp_sched_guided is required.
 *
...
void parallel_itmv_mult(int threadcnt, int mappingtype, int chunksize) {
  ...
  omp_set_schedule(mappingtype, chunksize);
